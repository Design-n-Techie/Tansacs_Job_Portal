...
async function loginUser(values) {
    const response = await  axios.post('http://52.66.175.54:8000/verified', values);
  ...
  const handleChange = (e, index) => {
    ...
    const sendotp = async () =>  {
        ...
        try {
            ...
            const response = await axios.post('http://52.66.175.54:8000/send-otp', formData);
...
<div className='mt-10'>
    ...
    <form onSubmit={ onSubmit}>
        ...
        <div>
            {error && <p className='text-red-600 font-bold mb-3'>Wrong OTP</p>}
        </div>
        ...
        <div className='my-5'>
             <p className='text-sm'>Did'nt receive OTP <small className='text-red-600'>{Math.floor(timer / 60)}:{timer % 60}</small></p>
        </div>
